...
def edit_password(request):
    ...
    login_model = request.dbsession.query(Users).filter(Users.id == userid).first()
    ...
    if login_model and check_password(input_password, hashed_pw):
        request.dbsession.query(Users).filter(Users.id == userid).update({Users.password: new_hash_password})
